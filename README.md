...
The repository has 26 focused MinGW checks, plus `audit-repo`:
...
`.github/workflows/mingw-validation.yml` runs the Windows/MinGW build, all 26 focused checks, and repository integrity audit for pull requests and pushes to `main`. Its Linux jobs validate asset-path case, perform a best-effort Linux build/smoke test, and run a required deterministic replay ASan/UBSan check.
...
|---|---|---|---|
| Windows / MinGW | Local and CI `make mingw`; portable Python DLL copy | 26 focused checks + audit in CI | Headless runtime checks |
